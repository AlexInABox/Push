    ...
    internal sealed class SettingValueReceived
    {
        ...
        private void TryToPush(Player player)
        {
            ...
            if (_pushCooldowns.TryGetValue(player.Id, out float lastPushTime) && currentTime - lastPushTime < 10f)
            {
                ...
                player.ShowHint(Push.Instance.Config.PlayerPushCooldown.Content + " " + remainingCooldown + " seconds remaining...", Push.Instance.Config.PlayerPushCooldown.Duration);
                return;
            }

            // Get the player's position and the direction they are facing
            Vector3 playerPosition = player.Position;
            Vector3 forwardDirection = player.Rotation * Vector3.forward; // Using player's rotation to determine forward direction

            // Check what the player is shooting at with a raycast
            if (!Physics.Raycast(playerPosition, player.Rotation * Vector3.forward, out RaycastHit raycastHit, 1f, ~(1 << 1 | 1 << 13 | 1 << 16 | 1 << 28)))
                return;
            ...
            {
                float angle = Vector3.Angle(player.Rotation * Vector3.forward, targetedPlayer.Rotation * Vector3.forward);
                const float MAX_ALLOWED_ANGLE = 45f; // Maximum angle (in degrees) to allow the push
                ...
                else
                {
                    ...
                    player.ShowHint(Push.Instance.Config.PlayerPushWrongAngle.Content, Push.Instance.Config.PlayerPushWrongAngle.Duration);
                }
            }
        }
        ...
        {
            const float PUSH_DISTANCE = 2f; // total push distance
